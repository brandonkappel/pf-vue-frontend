<template>
  <Dialog :visible="workoutStore.isCreateWorkoutDialogVisible" modal @update:visible="hideDialog"
    :header="workout ? workout.name || 'New Workout' : 'New Workout'" :style="{ width: '75vw', maxWidth: '720px' }"
    :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" pt:mask:class="backdrop-blur-sm">
    <div class="space-y-6 pb-20">
      ...
      <section class="space-y-4">
        <div v-for="(item, index) in workoutItems" :key="index"
          ...
          <div class="flex items-center justify-between px-4 py-3 bg-surface-50 dark:bg-surface-800">
            ...
            <div class="flex items-center gap-1">
              <Button v-if="item.type" @click="toggleEdit(index)" :label="item.readOnly ? 'Edit' : 'Done'"
                :icon="item.readOnly ? 'pi pi-pencil' : 'pi pi-check'"
                :severity="item.readOnly ? 'secondary' : 'success'" size="small" outlined />
              <Button @click="removeExercise(index)" icon="pi pi-trash" severity="danger" text rounded size="small" />
            </div>
          </div>
          ...
          <div v-if="!item.readOnly" class="px-4 py-4 space-y-4">
            ...
            <div>
              ...
              <div class="flex gap-2 flex-wrap">
                <button v-for="type in workoutTypes" :key="type" @click="item.type = type; typeChange(index)" :class="[
            ...
            <div v-if="item.type === 'Strength'" class="space-y-4">
              <div>
                ...
                <AutoComplete v-model="item.exercise" @option-select="onSelectedExercise" dropdown optionLabel="name"
                  :suggestions="exercises" @complete="search" class="w-full" placeholder="Search exercises..." />
              </div>
            ...
            <div v-if="item.type === 'Metcon'" class="space-y-4">
              ...
              <div>
                ...
                <div class="flex gap-2">
                  <button v-for="mt in metconTypes" :key="mt" @click="metconType = mt" :class="[
                    'px-4 py-2 rounded-lg border-2 text-sm font-semibold transition-all duration-150',
                    metconType === mt
                      ? 'border-primary bg-primary/10 text-primary'
                      : 'border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 text-surface-600 dark:text-surface-400 hover:border-surface-300'
              ...
              <div v-if="metconType === 'Generated Workout'" class="space-y-4">
                <div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button v-for="eq in availableEquipment" :key="eq" @click="toggleEquipment(item, eq)" :class="[
                ...
                <div>
                  ...
                  <div class="flex gap-2 flex-wrap">
                    <button v-for="type in scoreTypes" :key="type" @click="item.scoreType = type" :class="[
                      'px-4 py-2 rounded-lg border-2 text-sm font-semibold transition-all duration-150',
                      item.scoreType === type
                        ? 'border-primary bg-primary/10 text-primary'
                        : 'border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 text-surface-600 dark:text-surface-400 hover:border-surface-300'
                ...
                <div>
                  ...
                  <div class="flex gap-2">
                    <button v-for="d in durations" :key="d" @click="item.selectedDuration = d as string" :class="[
                      'px-4 py-2 rounded-lg border-2 text-sm font-semibold transition-all duration-150',
                ...
                <Button @click="generateWorkout(index)" label="Generate Workout" icon="pi pi-bolt" class="w-full" />
              </div>
          ...
          <div v-else-if="item.type" class="px-4 py-3 text-sm text-surface-500 dark:text-surface-400">
            ...
            <template v-else>
              ...
              <span v-if="item.description" class="ml-1">— {{ item.description.slice(0, 80) }}{{ item.description.length
                > 80 ? '...' : '' }}</span>
            </template>
          </div>
        ...
        <Button label="Add Workout Item" icon="pi pi-plus" severity="secondary" outlined class="w-full"
          @click="addItem" />
      </section>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        <Button :label="isNew ? 'Create Workout' : 'Save Changes'" icon="pi pi-check" @click="createWorkout" />
      </div>
    </template>
  </Dialog>
</template>
