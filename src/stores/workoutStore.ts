...
export const useWorkoutStore = defineStore('workout', {
  ...
  actions: {
    ...
    async saveRecentSearch(exercise: any) {
      ...
      try {
        const response = await axiosInstance.post(
          `/auth/users/${authStore.userId}/exercise-searched`,
          exercise,
          {
            headers: {
              skipLoading: true
    ...
    async getRecentlySearched() {
      ...
      try {
        const response = await axiosInstance.get(
          `/auth/users/${authStore.userId}/recent-searches`,
          {
            headers: {
              skipLoading: true
            }
    ...
    showCreateWorkoutDialog() {
      this.isCreateWorkoutDialogVisible === true
        ? (this.isCreateWorkoutDialogVisible = false)
        : (this.isCreateWorkoutDialogVisible = true);
    }
  }
});
